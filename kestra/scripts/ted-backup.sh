...
# ---------------------------
# Copy backup from PostgreSQL
# container to local storage
# ---------------------------
...
if [ $? -ne 0 ]; then
    echo "WARNING: Network backup succeeded, but local backup could not be deleted."
    echo "Local backup:"
    echo "$LOCAL_BACKUP"
